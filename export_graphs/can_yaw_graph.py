    ...
    for i in range(len(gps_vss_100ms)):
        ...
        if min_vss_3 >= 10:
            ...
            if sum(total_rotation) >= 60.0:
                sudden_rotation.append([i/10, f'Rotation:{sum(total_rotation):.2f}°, Vel:{min_vss_3}~{max_vss_3}'])
...
def save_graphs(json_data_list):
    ...
    for data in json_data_list:
        ...
        plt.tight_layout()
        # plt.show()
        plt.savefig(f'{json_root}/{target_dir}/yaw_graph_{json_filename[:-5]}.png')
        plt.close()
